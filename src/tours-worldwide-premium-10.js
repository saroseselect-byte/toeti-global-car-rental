import supplierRoutes from './tours-worldwide-worker.js';
...
const I={
  ...
  czech:'https://commons.wikimedia.org/wiki/Special:Redirect/file/Prague_Charles_Bridge_Tower_View_West-06.jpg',
  greece:'https://www.goodfreephotos.com/albums/greece/crete/coastline-of-crete-greece.jpg',
  zanzibar:'https://commons.wikimedia.org/wiki/Special:Redirect/file/ZanzibarBeach.jpg',
  indonesia:'https://commons.wikimedia.org/wiki/Special:Redirect/file/Lombok_Coast.jpg',
  fiji:'https://commons.wikimedia.org/wiki/Special:Redirect/file/Fiji.JPG',
  mauritius:'https://commons.wikimedia.org/wiki/Special:Redirect/file/East_coastline_of_Mauritius_(53697992628).jpg',
  samoa:'https://commons.wikimedia.org/wiki/Special:Redirect/file/Samoa_beach.jpg'
};
...
function page(){return `<!doctype html><html><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><title>TOETI Worldwide · Cars & Experiences</title><style>${S}</style></head><body><nav class=nav><a class=brand href=/tours>TOETI · WORLDWIDE</a><div class=right><a href=#world class=en>Explore destinations</a><a href=#world class=nl>Ontdek bestemmingen</a><span class=lang><button onclick="setL('nl')">NL</button>|<button onclick="setL('en')">EN</button></span></div></nav><section class=hero><div><div class=ey>TOETI · CARS & EXPERIENCES</div><h1 class=en>Real places.<br>Local partners.</h1><h1 class="nl block">Echte plekken.<br>Lokale partners.</h1><p class=en>Choose the country first. Every supplier below is grouped under the destination where the service is actually delivered.</p><p class="nl block">Kies eerst het land. Iedere leverancier hieronder staat bij de bestemming waar de dienst werkelijk wordt uitgevoerd.</p></div></section><main id=world>${C.map(c=>`<section class=country style="background-image:url('${I[c[0]]}')"><div class=inner><div class="ey en">${c[3]}</div><div class="ey nl">${c[4]}</div><h2 class=en>${c[1]}</h2><h2 class="nl block">${c[2]}</h2><p class=en>Cars and experiences from local partners in this destination. Open a partner page to view the current offer and request availability.</p><p class="nl block">Auto's en ervaringen van lokale partners op deze bestemming. Open een partnerpagina om het actuele aanbod te bekijken en beschikbaarheid aan te vragen.</p><div class=links>${c[5].map(x=>`<a class=btn href="${x[1]}">${x[0]} →</a>`).join('')}</div></div></section>`).join('')}</main><footer class=footer><b>TOETI Worldwide</b><p class=en>Correct destination. Correct local partner. One clear journey.</p><p class=nl>De juiste bestemming. De juiste lokale partner. Eén duidelijke reis.</p></footer><script>function setL(l){localStorage.setItem('toeti_lang',l);document.body.dataset.lang=l;document.documentElement.lang=l}let q=new URLSearchParams(location.search).get('lang'),s=q||localStorage.getItem('toeti_lang')||((navigator.language||'en').toLowerCase().startsWith('nl')?'nl':'en');setL(s)</script></body></html>`}
export default{async fetch(r,env,ctx){const u=new URL(r.url);if(u.pathname==='/'||u.pathname==='/tours'||u.pathname==='/experiences')return new Response(page(),{headers:{'content-type':'text/html;charset=UTF-8','cache-control':'no-store'}});return supplierRoutes.fetch(r,env,ctx)}};
